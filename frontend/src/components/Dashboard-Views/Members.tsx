...
const Members = () => {
  ...
  const ADD_MEMBER_ENDPOINT = import.meta.env.VITE_APP_NODE_ENV === "production"
  ...
  const renderMemberInfo = () => {
    if (member) {
      ...
      return (
        ...
        <button onClick={() => toggleMemberInfo(false, "")} className="item-3 ml-auto"></button>
  ...
  const renderInviteMenu = () => {
    if (showInviteMenu) {
      ...
      ref={ref} 
        ...
        <button onClick={() => toggleInviteMenu(false)} className="item-3 ml-auto"></button>
          <h1 className="text-2xl mb-3 text-center w-full">
          Invite to {orgName}
          </h1>
          {invited ? <p className="text-yellow-500 text-center w-full mb-1"> {msg} </p> : <p className="text-red-500 text-center w-full mb-1"> {msg} </p>}
          ...
          <button onClick={() => handleInvite()} className="w-full p-3 rounded-xl text-white bg-[#606060]"> Invite Member </button>
          </motion.div>
      )
    }
  }
          ...
          members.map((member, index: number) => {
            return (
              <div
                key={index}
                onClick={() =>
                  toggleMemberInfo(true, member.userId)
                }

...
import { getProjectInfo, removeMember, fetchDevicesHandler, fetchIntradayDataHandler, fetchDataHandler, downloadDataHandler } from '../controllers/ProjectController';
...
describe('ProjectController', () => {
    ...
    describe('removeMember', () => {
        it('should remove a member successfully', async () => {
            ...
            expect(mockProject.updateOne).toHaveBeenCalledWith({ $pull: { members: mockUser._id } });
        ...
        it('should return 400 if trying to remove the owner', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith({ msg: 'Cannot remove owner from project' });
        });
    ...
    describe('fetchDevicesHandler', () => {
        ...
        it('should return 400 if required parameters are missing', async () => {
            ...
            expect(res.status).toHaveBeenCalledWith(400);
            expect(res.json).toHaveBeenCalledWith({ message: 'Fitbit account not linked to project' });
        });
    ...
    describe('fetchIntradayDataHandler', () => {
        it('should return intraday data if the required parameters are provided', async () => {
            ...
            req.project = mockProject;
            req.query = { dataType: 'steps', date: '2023-01-01', detailLevel: '1min' };

            await fetchIntradayDataHandler(req as Request, res as Response);

            expect(fetchIntradayData).toHaveBeenCalledWith('user123', 'token123', 'steps', '2023-01-01', '1min');
        ...
        it('should return 500 if there is a server error', async () => {
            (fetchIntradayData as jest.Mock).mockRejectedValue(new Error('Server error'));
    ...
    describe('fetchDataHandler', () => {
        it('should return data if the required parameters are provided', async () => {
            ...
            expect(fetchData).toHaveBeenCalledWith('user123', 'token123', '2023-01-01', '2023-01-31');
            expect(res.json).toHaveBeenCalledWith(mockData);
        });
    ...
    describe('downloadDataHandler', () => {
        it('should return a CSV file if the required parameters are provided', async () => {
            ...
            req.project = mockProject;
            req.query = { deviceId: 'device1', dataType: 'steps', date: '2023-01-01', detailLevel: '1min' };

            await downloadDataHandler(req as Request, res as Response);

            const expectedCSV = '2023-01-01T00:00:00Z,100\n';
            expect(fetchIntradayData).toHaveBeenCalledWith('user123', 'token123', 'steps', '2023-01-01', '1min');
            expect(res.setHeader).toHaveBeenCalledWith('Content-Disposition', 'attachment; filename="device1-2023-01-01-1min.csv"');

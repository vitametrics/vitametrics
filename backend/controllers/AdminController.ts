    ...
    try {
      ...
      if (!user) {
        ...
        if (role === 'tempUser') {
          ...
          const fitbitAuthLink = `${process.env.BASE_URL}/api/auth?userId=${newUserId}&projectId=${project.projectId}&tempUser=true`;
          if (process.env.NODE_ENV === 'production') {
            await sendEmail({
              to: email,
            ...
            logger.info(
              `Temp user invited to join project: ${project.projectName}. They can set their password by following this link: ${fitbitAuthLink}`
            );
            res.status(200).json({ msg: 'Temp member added successfully', project });
            return;
          }
          ...
          if (process.env.NODE_ENV === 'production') {
            await sendEmail({
              ...
              text: `You have been invited to join the project: ${project.projectName}. Please set your password by following this link: ${process.env.BASE_URL}/set-password?token=${newUser.setPasswordToken}&projectId=${project.projectId}`,
            });
          } else {
            logger.info(
              `User invited to join project: ${project.projectName}. They can set their password by following this link: ${process.env.BASE_URL}/set-password?token=${newUser.setPasswordToken}&projectId=${project.projectId}`
            );

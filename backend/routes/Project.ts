...
]), checkProjectMembership, refreshFitbitToken, asyncHandler(fetchDataHandler));
...
]), checkProjectMembership, refreshFitbitToken, asyncHandler(fetchIntradayDataHandler));
...
router.get('/download-data', verifySession, validationHandler([
    ...
    query('date').not().isEmpty().withMessage('Date is required'),
    query('detailLevel').not().isEmpty().withMessage('Detail level is required')
]), checkProjectMembership, refreshFitbitToken, asyncHandler(downloadDataHandler));
